...
public class TrustedDeviceAuthenticator implements Authenticator, CredentialValidator<TrustedDeviceCredentialProvider> {
    ...
    public TrustedDeviceAuthenticator(KeycloakSession session) {
        this.session = session;
    }

    @Override
    public void authenticate(AuthenticationFlowContext context) {
        TrustedDeviceCredentialModel credential = validateCookie(context.getUser());
        if (credential != null) {
            context.success();
        } else {
    ...
    public TrustedDeviceCredentialModel validateCookie(UserModel user) {
        TrustedDeviceToken deviceToken = getCookie();
        if (deviceToken == null) {
            return null;
        }

        return user.credentialManager().getStoredCredentialsByTypeStream(TrustedDeviceCredentialModel.TYPE_TWOFACTOR)
    ...
    public TrustedDeviceToken getCookie() {
